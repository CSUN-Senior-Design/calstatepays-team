#!/bin/bash

sudo adduser calstatepays || true

sudo -u calstatepays mkdir ~/.ssh || true
sudo -u calstatepays chmod 700 ~/.ssh || true
sudo -u calstatepays touch ~/.ssh/authorized_keys || true
sudo -u calstatepays chmod 600 ~/.ssh/authorized_keys || true

echo "ssh-rsa  vibe@LoopyD" >> ~/.ssh/authorized_keys
echo "ssh-rsa  edgargodoy@Edgars-MacBook-Pro.local" >> ~/.ssh/authorized_keys
echo "ssh-rsa  bconti00@Bconti" >> ~/.ssh/authorized_keys
echo "ssh-rsa " >> ~/.ssh/authorized_keys

exit